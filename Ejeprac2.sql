...
--	 Cotizacion    	FLOAT defult 99,
	 Capital       	INT ,
	 PRIMARY KEY    (NombreE),
--	 CHECK          (Capitol > 0)
	 CHECK          (Capital > 0)
		);
-- NOTA : hay dos errores, el 1º no esta en la linea dos 
-- ERROR : 'defult' es 'default'
--         'Capitol' mal escrito 'Capital'
---------------------------------------------------------------------
...
-- Telefono	CHAR(12)
PRIMARY KEY (DNI));
-- NOTA : no es que falte parentesis, es otro signo de puntuacion
...
create table Tarjeta
 ...
 PRIMARY KEY (NumT),
-- CHECK TipoT in ('VXK','COSA','PISSA')
 CHECK (TipoT in ('VXK','COSA','PISA'))
 );
-- ERROR1: nombre del atributo del check
-- ERROR2: (se descubre mas adelante) tipo es 'pisa', no 'pissa'
-- ERROR3: la condicion del check del tipot va toda entre paréntesis
---------------------------------------------------------------------
...
-- ERROR1 : nombre atrib clave ajena no existe
-- ERROR2  : (se ve mas adelante) falta el atrib caducidad
...
-- ERROR: 
--	(para continuar comente insert con '--')
-- repetida la clave, como es la misma tupla que la primera la quito
---------------------------------------------------------------------
...
-- o necesito modificar Invierte en la declaracion? 
...
-- ERROR:que restriccion es? ver a la declaracion de Tarjeta
-- el tipo es 'pisa', arreglar en la declaracion de la tabla 
INSERT INTO Tarjeta VALUES ('30000002', 'PISA','MASTERUIN');
-- INSERT INTO Tarjeta VALUES ('00000000','PISA','MASTERUIN');
-- ERROR que restriccion es? ver a la declaracion de Tarjeta
-- (para continuar comente insert con '--')                  
-- el numt no puede ser '0'
...
-- ERROR: consultar la declaracion de la tabla : arreglarla
-- ver arreglo alli
...
-- ERROR (para continuar comente insert con '--') 
